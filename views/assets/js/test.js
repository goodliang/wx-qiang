...
//             ball.diameter = self.diameter;
//             ball.radius = self.radius;
//             $(ball).css('webkitTransform', 'translate(' + (Math.random() * self.R_bound) + 'px,' + (Math.random() * self.B_bound) + 'px)'); //球的初始位置，
//             ball.vx = Math.random() * 6 - 3;
//             ball.vy = Math.random() * 6 - 3;
//             frag.appendChild(ball);
//             self.balls[i] = ball;
...
//         var self = this;
//         ball.vy += self.gravity;
//         $(ball).css('webkitTransform', 'translate(' + (ball.offsetLeft + ball.vx) + 'px,' + (ball.offsetTop + ball.vy) + 'px)');
//         //判断球与窗口边界相碰，把变量名简化一下
//         var L = self.L_bound,
//             R = self.R_bound,
//             T = self.T_bound,
...
//             BC = self.bounce;
//         if (ball.offsetLeft < L) {
//             $(ball).css('webkitTransform', 'translateX(' + L + 'px)');
//             ball.vx *= BC;
//         } else if (ball.offsetLeft + ball.diameter > R) {
//             $(ball).css('webkitTransform', 'translateX(' + (R - ball.diameter) + 'px)');
//             ball.vx *= BC;
//         } else if (ball.offsetTop < T) {
//             $(ball).css('webkitTransform', 'translateY(' + T + 'px)');
//             ball.vy *= BC;
//         }
//         if (ball.offsetTop + ball.diameter > B) {
//             $(ball).css('webkitTransform', 'translateY(' + (B - ball.diameter) + 'px)');
//             ball.vy *= BC;
//         }
//     }
// }
